...
void SmallShell::executeCommand(const char *cmd_line) {
    ...
    try {
        try {
            ...
            cerr << "smash error: kill: job-id "
                 << kill_job_doesnt_exist.job_id
                 << " does not exist" << endl;
            throw;
        }
...
int JobEntry::run_time() const {
    return difftime(time(nullptr), cmd->create_time);
}
...
void JobsList::removeFromStopped(JobId job_id) {
    for (auto job_id_it = last_stopped_jobs.begin();
         job_id_it != last_stopped_jobs.end();) {
        if (*job_id_it == job_id)
...
ChangeDirCommand::ChangeDirCommand(const char *cmd_line, vector<string> &args)
        : BuiltInCommand(cmd_line) {
    SmallShell &smash = SmallShell::getInstance();
...
ForegroundCommand::FGJobIDDoesntExist::FGJobIDDoesntExist(JobId job_id)
        : job_id(job_id) {}
...
void ForegroundCommand::execute() {
    ...
    smash.replace_fg_and_wait(
            JobEntry(nullptr, job->pid)); //job->cmd will delete from joblist
}
...
BackgroundCommand::BGJobIDDoesntExist::BGJobIDDoesntExist(JobId job_id)
        : job_id(job_id) {}

BackgroundCommand::BGJobAlreadyRunning::BGJobAlreadyRunning(JobId job_id)
        : job_id(job_id) {}
...
KillCommand::KillCommand(const char *cmdLine1, vector<string> &args)
        : BuiltInCommand(cmdLine1) {
...
KillCommand::KillJobIDDoesntExist::KillJobIDDoesntExist(JobId job_id) : job_id(
        job_id) {}
...
RedirectionCommand::RedirectionCommand(const char *cmd_line_c,
        ...
        Command(cmd_line_c, _isBackgroundComamnd(cmd_line_c)),
        IO_type(IO_type) {
